...
export default function setRailCovers() {
    state.rails.forEach((rail) => {
        ...
        reference[rail.name]["dir-versions"][rail.direction]["end-points"].forEach((endPointRef) => {
            let [railCoverX, railCoverY] = endPointRef["rail-cover-coord-offset"],
                railCoverGlobalCoordX = x + railCoverX,
                railCoverGlobalCoordY = y + railCoverY,
                railCoverDirection = endPointRef["rail-cover-dir"],
                isEndOfLine = endPointRef["rail-path-vars"].every((railPathVar) => {
                    let [railPathVarX, railPathVarY] = railPathVar["rail-coord-offset"],
                        nextRails = getStructuresAt(x + railPathVarX, y + railPathVarY);
